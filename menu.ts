...
class Menu {
    ...
    constructor(public config: Config) { 
        ...
        this.$activeBar = <HTMLElement>document.getElementsByClassName(this.activeBarClass)[0];
        ...
        console.log("Menu loaded");
            ...
            for(var mi = 0; mi < this.menuItems.length; mi++){
                var $li:HTMLElement = <HTMLElement>this.menuItems.item(mi);
    ...
    * Blur each menu item
    */
    public blurMenuItems(){
        console.log("enter blurMenuItems "+this.menuItems);
        for(var mi = 0; mi < this.menuItems.length; mi++){
            (<HTMLElement>this.menuItems.item(mi)).blur();
        }
    }
    //end blurMenuItems
   ...
   */
    public notActiveBar(li : HTMLElement) {
   ...
   */
    public slide(i : number, $el : HTMLElement) {
        ...
        if($el.getAttribute('data-active-bar') !== 'false' && this.turnedOn) {
          // in all browsers, provides X movement
            (<HTMLElement>this.$menu).className = this.direction + ' directional-nav pos-' + i;
   ...
   */
    public changePage(i:number, $el:HTMLElement) {
        ...
        var parentLiElement = <HTMLElement>$parentLi;
